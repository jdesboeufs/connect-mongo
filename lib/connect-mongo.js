  ...
  function MongoStore(options, callback) {
    ...
    if (options.mongoose_connection){
      ...
      if(options.mongoose_connection.db.serverConfig.servers){ // using elastic mongodb
        options.host = options.mongoose_connection.db.serverConfig.servers[0].host;
        options.port = options.mongoose_connection.db.serverConfig.servers[0].port;
        options.options = options.mongoose_connection.db.serverConfig.servers[0].options;
      }
      else{
        options.host = options.mongoose_connection.db.serverConfig.host;
        options.port = options.mongoose_connection.db.serverConfig.port;
        options.options = options.mongoose_connection.db.serverConfig.options;
      }

      this.db = new mongo.Db(options.mongoose_connection.db.databaseName,
                             new mongo.Server(options.host, options.port, options.options),
