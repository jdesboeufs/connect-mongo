...
var MongoStore = module.exports = function MongoStore(options, callback) {
  ...
  if (Array.isArray(options.host) && Array.isArray(options.port)) {
    var servers = [];
    for(var i = 0; i < options.host.length; i++) {
      servers.push(new mongo.Server(options.host[i], options.port[i],
    ...
    this.db = new mongo.Db(options.db, set);
  } else {
