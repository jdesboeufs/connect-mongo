...
var MongoStore = module.exports = function MongoStore(options, callback) {
  	...
  	if(!options.url) 
	{
		...
		{
			obj.host = (obj.host || defaultOptions.host);
	  		obj.port = (obj.port || 27017);
	  		obj.db = (obj.db || defaultOptions.db);
		...
		options.url = generate_mongo_url(options);
	}
  ...
  this._get_collection = function(callback) {
    if (self.collection && Db.serverConfig._serverState == "connected") {
      callback && callback(self.collection);
		...
		{
		  Db = data;
	      Db.collection(self.db_collection_name, function(err, collection) {
	        if (err) {
